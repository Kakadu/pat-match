...
What would be solutions for \mintinline{scheme}{q}, that is not equal \mintinline{scheme}{(s (s (s ,v))))} for any possible v?
...
\footnotetext[2]{Jin, G. Rosenblatt, M. Might, and Zhang L. 2021. Universal Quantification
...
\begin{minted}{ocaml}
let straightforward q rez =       (* OCanren *)
  conde
    [ fresh ()
        (rez === !!1)
        (q === Std.pair !!true __)
...
\only<2>{
  ...
  \end{minipage}
  \begin{itemize}
  \item [\faBad] For \mlinline{fresh (v) (q === Std.pair !!true v)} both 1 and 2m because variable \mlinline{tmp} is existential.
