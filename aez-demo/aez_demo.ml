...
let test1 () =
  ...
  let f1 = F.make_lit F.Eq [ fx3; tu ] in
  (* f(x + 3) = u *)
  let f2 = F.make_lit F.Eq [ fy2; tw ] in
  (* f(y + 2) = w *)
  let f3 = F.make_lit F.Eq [ tx; T.make_arith T.Minus ty t1 ] in
  (* x = y - 1 *)
  let neg_goal = F.make F.Not [ F.make_lit F.Eq [ tu; tw ] ] in
  ...
  with Unsat _ -> print_endline "valid"
...
let test2 () =
  ...
  let tu = T.make_app u [] in
  let tw = T.make_app w [] in
  let f1 = F.make_lit F.Lt [ T.make_int (Num.Int 0); tx ] in
  (* 0 < x*)
...
let test3 () =
  ...
  | Error (DuplicateSymb s) ->
      Printf.printf "DuplicateSymb %s\n%!" (Hstring.view s)
  | Error (DuplicateTypeName s) ->
      Printf.printf "DuplicateTypeName %s\n%!" (Hstring.view s)
  | Error (UnknownType s) ->
      Printf.printf "unknown type '%s'. \n%!" (Hstring.view s)
  | Error (UnknownSymb s) ->
      Printf.printf "unknown symb %s %d\n%!" __FILE__ __LINE__
