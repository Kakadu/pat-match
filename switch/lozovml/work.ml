...
let rec list_assoc name ys =
  match ys with
  | (k, v) :: xs -> if k = name then v else list_assoc name xs
;;
...
let list_decorate_nat xs = list_combine (list_mapi (fun i x -> i) xs) xs

let rec list_nth_nat idx xs =
  match idx, xs with
  | Z, x :: _ -> x
...
let rec eval_pat s pats =
  match pats with
  | [] -> None
  | (p, rhs) :: ps -> if match1pat s p then Some rhs else eval_pat s ps
;;
...
let tinfo_names tt =
  match tt with
  | T xs -> list_map fst xs
;;

let tinfo_names_with_arity tt =
  match tt with
  | T xs -> list_map (fun p -> fst p, list_length (snd p)) xs
;;

let tinfo_args tt name =
  match tt with
  | T xs -> list_assoc name xs
;;

let tinfo_nth_arg tt n =
  match tt with
  | T xs -> list_nth_nat n xs
;;

let info_assoc tt name =
  match tt with
  | T xs -> list_assoc name xs
;;
...
let rec eval_m s typinfo0 path0 =
  ...
  | ans, info -> ans, tinfo_names_with_arity info
;;
