...
let eval_examples (module W : WORK) (module Arg : ARG_FINAL) =
  ...
  let demo_exprs =
    ...
    |> List.map (fun e ->
           ...
           in
           let rez =
             (OCanren.Stream.hd stream)#reify (Std.Option.reify IR.reify) in
           (e, rez) ) in
  demo_exprs
...
module Make (W : WORK) (Arg : ARG_FINAL) = struct
  ...
  (** synthetizer main  *)
  let work ?(n = 10) ~with_hack ~print_examples ~check_repeated_ifs:_
      ~debug_filtered_by_size ~prunes_period ~with_default_shortcuts =
    print_endline
      "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%";
    let printed_clauses =
      Format.asprintf "%a" Clauses.pretty_print Arg.clauses in
    ...
    let () =
      ...
      | Some a ->
          ...
          assert (Arg.max_ifs_count <= IR.count_ifs_ground a) in
   ...
   *)
      in
      helper ~height:0 ~count:0 [] root in
               ...
               | _ ->
                   ...
                   let () =
                     if OCanren.Stream.is_empty stream then
                       failwith "Bad (?) example" in
                   let rez =
                     (OCanren.Stream.hd stream)#reify
                       (Std.Option.reify IR.reify) in
                   (e, rez) ) in
      let () =
        if print_examples then
          demo_exprs
          |> List.iter (fun (e, rez) ->
                 ...
                 Format.printf "%s\n%!"
                 @@ GT.show Std.Option.logic IR.show_logic rez ) in
      List.map (fun (e, _) -> Expr.inject e) demo_exprs in
    ...
    let () =
      ...
      | Some n ->
          let open OCanren.PrunesControl in
          set_max_skips n; enable_skips ~on:true; reset ()
      | None -> disable_periodic_prunes () in
