...
module Make (Arg : ARG_FINAL) = struct
  let work ?(n = 10) ~with_hack ~print_examples ~check_repeated_ifs
    ...
    let count_if_constructors : IR.logic -> int =
     fun root ->
      let next_seen seen scru tag =
        if not check_repeated_ifs then seen
        else
          match Matchable.to_ground scru with
          | Some mground ->
              if List.mem mground seen then raise FilteredOutByForm
              else mground :: seen
          | _ -> seen
      in
      let rec helper ~height ~count seen = function
        ...
        | Value (Switch (scru, xs, on_default)) ->
            ...
            let () =
              if height > Arg.max_nested_switches then
                raise FilteredOutByNestedness
            in
            GT.foldl Std.List.logic
              ...
              0 xs
            + helper ~height
                ~count:(count + logic_list_len_lo xs)
                seen on_default
      in
    ...
    let _ifs_size_hack (ans : IR.injected) =
      ...
      structural ans IR.reify (fun (ir : IR.logic) ->
          ...
          let debug fmt =
            Format.ksprintf
              (fun s ->
                if _do_debug && verbose then Format.printf "%s" s else ())
              fmt
          in
          try
          ...
          with
          | FilteredOutBySize n ->
              if debug_filtered_by_size then
                Format.printf
    ...
    let injected_exprs =
      let demo_exprs =
        run one
          (fun q -> Arg.inhabit Arg.max_height q)
               ...
               | Some ans ->
                     ...
                     OCanren.(run one)
                       (fun ir ->
                         fresh (n rez)
                           ...
                           (ir === IR.int n)
                           (Work.eval_ir scru_demo max_height typs
                              simple_shortcut0 simple_shortcut
                              simple_shortcut_tag answer_demo
                              (Std.Option.some n)))
                       (fun r -> r)
                   in
                  ...
                  *)
                   ...
                   if print_examples then
                     Format.printf "%s\n%!"
                     @@ IR.show_logic
                          ((OCanren.Stream.hd stream)#reify IR.reify))
      in
    ...
    in
    let info = Format.sprintf "fair lozovML II (%s)" Arg.info in
    ...
    let open Mytester in
    runR IR.reify on_ground on_logic n q qh
      ( info,
        fun ideal_IR ->
    ...
    Format.printf "Total synthesis time: %s\n%!"
      (let ms = Mtime.Span.to_ms span in
       if ms > 10000.0 then Format.sprintf "%10.0fs \n%!" (Mtime.Span.to_s span)
