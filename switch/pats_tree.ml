  ...
  let add_full_path tr p =
    ...
    let pp fmt t =
      ...
      |> Trie.iter (fun k set ->
             ...
             Stdlib.List.iter
               (fun n ->
                 Format.fprintf fmt "[%a]" (GT.fmt GT.int) (Unn_pre.N.to_int n))
               k;
  ...
  let rec f acc typs cur_path (pattern : Pattern.ground) =
    ...
    | PConstr (cname, xs) ->
        ...
        Format.printf "%d pats: %s\n%!"
          ...
          *)
        ground_list_fold2i_exn
          (fun acc i arg argtyp ->
            let next_path = cur_path @ [ N.of_int i ] in
            f acc argtyp next_path arg)
          acc xs arg_typs
  in
  List.fold_left (fun acc (p, _) -> f acc typs0 [] p) empty clauses
