...
module Tag = struct
  ...
  let all_tags =
    [ "zero"; "succ"; "true"; "false"; "nil"; "cons"; "nil2"; "pair"; "A"; "B"
    ; "C"; "int"; "triple"; "D"; "Push"; "Search"; "Ldi"; "Pushenv"; "Popenv"
    ; "Extend"; "Apply"; "Mkclos"; "Mkclosrec"; "IOp"; "Test"; "Clo"; "Int"
    ; "Val"; "Env"; "Code"; "42" ]
...
let rec inject_ground_list (xs : ('a, 'b) OCanren.injected Std.List.ground) :
  ...
  let rec helper = function
    ...
    | Std.List.Cons (x, xs) -> Std.List.cons x (helper xs) in
  helper xs
    ...
    let rec helper : (Tag.logic, logic Std.List.logic) t -> string = function
      ...
      | PConstr (s, ps) ->
          Printf.sprintf "(%s %s)" (GT.show Tag.logic s)
            (GT.show Std.List.logic (GT.show OCanren.logic helper) ps) in
...
module Expr = struct
  ...
  let height : ground -> int =
    let rec helper (EConstr (_, xs)) =
      GT.foldl Std.List.ground (fun acc x -> max acc (helper x)) 0 xs + 1 in
    helper
  ...
  let show : ground -> string =
    let rec helper pars x =
      ...
      | EConstr (s, xs) ->
          Printf.sprintf "%s %s" (GT.show Tag.ground s)
            (GT.show Std.List.ground (helper false) xs) in
    helper false
  ...
  let rec show_logic (x : logic) : string =
    let rec helper x =
      ...
      | EConstr (s, xs) ->
          Printf.sprintf "(%s %s)" (GT.show Tag.logic s)
            (GT.show Std.List.logic show_logic xs) in
  ...
  let inject (e : ground) : injected =
    let rec helper = function
      | EConstr (t, xs) ->
          constr (Tag.inject t)
            (inject_ground_list @@ GT.gmap Std.List.ground helper xs) in
    helper e
end
...
module Matchable = struct
  ...
  let show x =
    let rec helper = function
      | Scru -> "S"
      | Field (n, r) -> Printf.sprintf "%s[%s]" (helper r) (N.show n) in
    helper x
  ...
  let inject : ground -> injected =
   fun root ->
    let rec helper = function
      | Scru -> scru ()
      | Field (n, prev) -> field (N.inject n) (helper prev) in
    helper root
  ...
  let to_ground l =
    let rec helper = function
      ...
      | Value (Field (n, m)) ->
          N.to_ground n >>=? fun n -> helper m >>=? fun m -> Some (Field (n, m))
      | Var (_, _) -> None in
    helper l
          ...
          GT.foldl Std.List.logic
            (GT.foldl OCanren.logic (fun () (tag, irl) ->
                 Format.fprintf fmt "@[ | %a@ ->@ %a@]" (GT.fmt Tag.logic) tag
                   fmt_logic irl ) )
            () xs;
          Format.fprintf fmt "@[ | _ -> %a@])" fmt_logic default in
      ...
      | Switch (m, xs, default) ->
          let clauses =
            ...
            @@ GT.gmap Std.List.ground
                 ...
                 xs in
          switch (Matchable.inject m) clauses (helper default) in
    helper
   ...
   fun ps ->
    let rec one : Pattern.ground -> _ = function
      ...
      | PConstr (name, ps) ->
          ...
          @@ inject_ground_list
          @@ GT.gmap Std.List.ground one ps in
    Std.List.list
...
module Typs = struct
  ...
  let rec inject (e : ground) : injected =
    let rec helper : _ -> _ = function
      | s, xs ->
          t_item (Tag.inject s)
            (inject_ground_list @@ GT.gmap Std.List.ground inject xs) in
    match e with
