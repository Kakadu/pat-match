...
module N = struct
  ...
  let reify : (injected, logic) Reifier.t = reify
...
module Tag = struct
  ...
  type logic = GT.int OCanren.logic
    [@@deriving gt ~options:{ foldl; compare }]
  type injected = (ground) OCanren.ilogic
  ...
  let string_of_tag_exn tag =
    let n = to_int tag in
    match Stdlib.List.nth all_tags n with
    | s -> s
    | exception Not_found ->
...
module Pattern = struct
  [%%distrib
    type nonrec ('a,'b) t = ('a,'b) W.gpattern =
    | WildCard
    | PConstr of 'a * 'b
...
module Matchable = struct
  ...
  let rec show_logic x =
    ...
    | Scru        -> "S"
    | Field (n,r) ->
      Printf.sprintf "%s[%s]" (show_logic r) (N.show_logic n)
    in
    GT.show OCanren.logic helper x
...
module Clauses = struct
  ...
  let inject : pre_ground -> injected =
    let rec one : Pattern.ground -> Pattern.injected = function
    | WildCard -> Pattern.wc ()
    | PConstr (name,ps) ->
  ...
  let pretty_print ch clauses =
    ...
    clauses |> Stdlib.List.iter (fun (p,ir) ->
 ...
 *)
  ...
  type nonrec ('a, 'b, 'c) injected = ('a * 'b * 'c) OCanren.ilogic

  let reify : ('a, 'b) Reifier.t -> ('c, 'd) Reifier.t -> ('e, 'f) Reifier.t ->
       ...
       ( ra >>= fun fa ->
         ...
         rc >>= fun fc ->
          let rec foo = function
              ...
              | Value x -> Value (GT.gmap ground fa fb fc x)
          in
...
module Cases = struct
  ...
  type injected = (Tag.injected, IR.injected) Std.Pair.injected Std.List.injected
end
...
module type WORK = sig
  val eval_pat : Expr.injected -> Clauses.injected -> (IR.injected) Std.Option.injected -> goal
  val eval_ir :  Expr.injected -> N.injected -> Typs.injected ->
    (Matchable.injected -> N.injected -> Cases.injected  -> bool_inj -> goal) ->
    (Tag.injected -> Matchable.injected -> Cases.injected  -> (Matchable.injected) Std.List.injected ->
        bool_inj -> goal ) ->
    (CNames.injected -> Cases.injected ->  bool_inj -> goal ) ->
    IR.injected  ->
    (int OCanren.ilogic) Std.Option.injected -> goal

  val matchable_leq_nat: Matchable.injected -> N.injected ->  bool_inj -> goal
  val not_in_history: Matchable.injected -> Matchable.injected Std.List.injected ->  bool_inj -> goal
  val info_assoc: Typs.injected -> Tag.injected -> (Typs.injected) Std.List.injected -> goal
...
module WorkHO : WORK = struct
  ...
  let eval_ir :
    ...
    ( (Matchable.injected -> goal) ->
      (N.injected -> goal) ->
      ( (Tag.injected * IR.injected)OCanren.ilogic Std.List.injected -> goal) ->
      bool_inj -> goal) ->
    (_ -> _ -> _ -> _ ->
      bool_inj -> goal) ->
    (_ -> _ -> _ -> goal) ->
    (IR.injected  -> goal) ->
    (int OCanren.ilogic) Std.Option.injected ->
    goal =
      Wrap.HO.eval_ir
  ...
  *)
  ...
  let well_typed_expr:
    ...
    (Typs.injected -> goal) ->
    bool_inj -> goal
    = Wrap.HO.well_typed_expr

  let well_typed_expr: Expr.injected -> Typs.injected -> bool_inj -> goal =
    fun e t r -> Wrap.HO.well_typed_expr ((===)e) ((===)t) r

  (*let compile_naively p ir =
    Wrap.compile_naively ((===)p) ir*)

  let eval_pat s p r = Wrap.HO.eval_pat ((===)s) ((===)p) r
  let matchable_leq_nat m n r = Wrap.HO.matchable_leq_nat ((===)m) ((===)n) r
  let not_in_history x xs r = Wrap.HO.not_in_history ((===)x) ((===)xs) r

  let info_assoc typs name rez = Wrap.HO.info_assoc ((===)typs) ((===)name) rez
