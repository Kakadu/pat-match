...
module IR = struct
  ...
  type injected =
    ( Matchable.injected,
      (Tag.injected, injected) Std.Pair.injected Std.List.injected,
      injected,
      GT.int ilogic )
    t
    ilogic
...
module type WORK = sig
  val eval_pat :
    Expr.injected -> Clauses.injected -> IR.injected Std.Option.groundi -> goal
  ...
  val eval_ir :
    ...
    IR.injected ->
    int OCanren.ilogic Std.Option.groundi ->
    goal
...
module WorkHO : WORK = struct
  ...
  let eval_ir :
      ...
      (N.injected -> goal) ->
      ((Tag.injected, IR.injected) Std.Pair.injected Std.List.injected -> goal) ->
      MatchableKind.injected ->
      goal) ->
