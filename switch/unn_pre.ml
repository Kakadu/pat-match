...
module N = struct
  type 'a gnat = 'a W.gnat =
    | Z
    | S of 'a
  [@@deriving gt ~options:{ show; fmt; foldl; gmap; compare }]

  type ground = ground gnat [@@deriving gt ~options:{ show; fmt; foldl; gmap; compare }]
...
module Pattern = struct
  ...
  let parented flg pp fmt x = if flg then Format.fprintf fmt "(%a)" pp x else pp fmt x
...
*)
  ...
  let count_wildcards : ground -> int =
    GT.transform
      ...
      (fun fself ->
        object
          method c_PConstr acc _ _ args = GT.foldl OCanren.Std.List.ground fself acc args
...
module Matchable = struct
  ...
  let rec reify env (x : injected) : logic = For_gmatchable.reify N.reify reify env x
    ...
    let rec helper = function
      ...
      | Switch (m, xs, default) ->
        let clauses =
          ...
          @@ GT.gmap
               Std.List.ground
               (fun (tag, code) -> Std.Pair.pair (Tag.inject tag) (helper code))
               xs
        in
...
module Clauses = struct
  ...
  let inject : pre_ground -> injected =
   fun ps ->
    ...
    in
    Std.List.list @@ List.map (fun (p, rhs) -> Std.Pair.pair (one p) (IR.inject rhs)) ps
 ;;
...
module Typs = struct
  ...
  type logic =
    (Tag.logic, logic Std.List.logic) Std.Pair.logic Std.List.logic t OCanren.logic
  ...
  let assoc_exn typs key =
    ...
    | None ->
      failwith
        (Printf.sprintf
           "Typs.assoc_exn: key '%s' not found in '%s'"
           (GT.show Tag.ground key)
           (GT.show ground typs))
    | Some x -> x
  ;;
  ...
  let rec construct (root : pre_typ) : ground =
    ...
    | T xs ->
      mkt
      @@ List.map
           (fun (p, xs) -> Tag.tag_of_string_exn p, Std.List.of_list construct xs)
           xs
  ;;
end
...
module EvalMRez = struct
  ...
  let show x =
    GT.(show Std.Pair.ground Expr.show (show Std.List.ground @@ show Tag.ground)) x
  ;;

  let show_logic x =
    GT.(show Std.Pair.logic Expr.show_logic (show Std.List.logic @@ show Tag.logic)) x
  ;;
...
module WorkHO : WORK = struct
  ...
  *)
  let well_typed_expr_height
      : N.injected -> Expr.injected -> Expr.injected -> Typs.injected -> bool_inj -> goal
    =
