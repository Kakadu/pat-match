   ...
   *)
    ...
    let examples : (Expr.injected -> goal) list =
      ...
      let inputs : (Expr.injected -> goal) list =
        let clauses =
          List.append Arg.clauses [ (Pattern.WildCard, IR.Lit 42) ]
        in
        snd
        @@ List.fold_left
      ...
      let () =
        if print_examples then
          ...
          |> List.iter (fun example_goal ->
                 let open Tester in
                 run_r Expr.reify Expr.show_logic (-1) q qh ("", example_goal);
                 let goal_interpret1 rez =
                   fresh scru (example_goal scru)
                 ...
                 let __ _ =
                   ...
                   [%tester
                     run_r IR.reify IR.show_logic (-1) (fun _q -> success)]
                 in
                 ())
      in
      inputs
    ...
    let shortcut1 etag m cases history typs rez =
      ...
      if with_default_shortcuts then
        default_shortcut etag m cases history typs rez
      else success
    in
