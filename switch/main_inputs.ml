...
module type ARG0 = sig
  ...
  val shortcut0 :
    Matchable.injected -> N.injected -> Cases.injected -> bool ilogic -> goal
...
let optimize_pair : IR.ground -> IR.ground =
 fun ir ->
  ...
  let rec helper = function
    ...
    | Switch (_, (t, code) :: [], _) when Tag.string_of_tag_exn t = "pair" ->
        helper code
    | Switch (m, xs, default) ->
...
let optimize_triple : IR.ground -> IR.ground =
 fun ir ->
  ...
  let rec helper = function
    ...
    | Fail -> Fail
    | Switch (_, [], default) -> helper default
    | Switch (_, (t, code) :: [], _) when Tag.string_of_tag_exn t = "triple" ->
        helper code
    | Switch (m, xs, default) ->
        let ys = GT.gmap Std.List.ground (fun (t, c) -> (t, helper c)) xs in
        Switch (m, ys, helper default)
...
module ArgPeanoSimple : ARG0 = struct
  ...
  let rec all_inhabitants (rez : N.injected) =
    conde
      [ rez === N.z; fresh (size_tl tl) (N.s tl === rez) (all_inhabitants tl) ]
