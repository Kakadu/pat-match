...
let run parsed =
  let typed, env = Typechecker.type_file parsed in

  let pbs = Typechecker.split_goals_and_cnf typed in
...
let () =
  let _abc = PA.mk_enum_type_decl Loc.dummy [] "abc" [ "a"; "b"; "c" ] in
  let _v =
    PA.mk_logic Loc.dummy Symbols.Ac []
      (PFunction ([], PPTexternal ([], "abc", Loc.dummy)))
  in
  let _ = PA.mk_var in
  let goal_1 =
    PA.mk_goal Loc.dummy "abc_goal_1"
      (PA.mk_not Loc.dummy (PA.mk_pred_eq Loc.dummy _v three))
  in
