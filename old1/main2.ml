...
module Pattern = struct
  ...
  let get_arities (pat : ground) =
    ...
    in
    try Some (helper ArityMap.empty pat) with Bad -> None
end
  ...
  let show x =
    let rec helper pars x =
      ...
      | EConstr (s, xs) when pars ->
          Printf.sprintf "(%s %s)" (GT.show GT.string s)
...
let generate_demo_exprs pats =
  let height =
    List.fold_left (fun acc p -> max acc (Pattern.height p)) 0 pats
  in
  ...
  let arity_map =
    ...
    match
      Pattern.get_arities (PConstr (default_name, Std.List.of_list id pats))
    with
...
let _ =
  run_exn
    (Format.asprintf "%a" (pp_maybe pp_rhs))
    1 q qh
    ( "answers",
      fun q ->
        checkAnswer
          (pair (leaf "aaa") (leaf "bbb"))
...
module Matchable = struct
  ...
  let scru = scru
  let field = field
  let rec reify env x = For_gmatchable.reify Nat.reify reify env x
...
module IR = struct
  ...
  type logic =
    (string OCanren.logic, Matchable.logic, logic, int OCanren.logic) gir
    OCanren.logic
  ...
  let show e =
    let rec helper = function
      ...
      | IFTag (str, m, th_, el_) ->
          Printf.sprintf "(iftag %S %s %s %s)" str (Matchable.show m)
            (helper th_) (helper el_)
    in
    helper e
    ...
    let rec helper = function
      ...
      | IFTag (ltag, m, th_, el_) ->
          Printf.sprintf "(iftag %s %s %s %s)"
            ...
            (Matchable.show_logic m) (show_logic th_) (show_logic el_)
...
let inject_patterns ps =
  let rec one : Pattern.ground -> _ = function
    | WildCard -> Pattern.wc ()
    | PConstr (name, ps) ->
        Pattern.constr !!name @@ inject_ground_list
  ...
  let injected_exprs =
    ...
    let demo_exprs =
      ...
      |> List.filter (fun e ->
             ...
             run one
               (fun ir ->
                 eval_pat (Expr.inject e) injected_pats (Std.Option.some ir))
               (fun r -> r)
    ...
    |> List.iter (fun e ->
           runR
             ...
             1 q qh
             ( Printf.sprintf "test_demo: `%s`" (Expr.show e),
               fun ir -> eval_pat (Expr.inject e) injected_pats ir ));
    print_newline ();
    List.map Expr.inject demo_exprs
  in

  runR IR.reify IR.show IR.show_logic n q qh
    ( "ideal_IR",
      fun ideal_IR ->
        let init =
          fresh (hack1 hack2) success
...
let eval_pat_hacky :
    Expr.injected -> IR.injected -> Clauses.injected -> IR.injected -> goal =
 fun expr_scru onfail pats res ->
  eval_pat_hacky (( === ) expr_scru) (( === ) onfail) (( === ) pats) res

let eval_ir_hacky : Expr.injected -> IR.injected -> IR.injected -> goal =
...
let () =
  let ps = [ (pnil, IR.eint 1); (pwc, IR.eint 2) ] in
  main ~n:0 ps;
  run_hacky ps
...
*)
  ...
  let inhabit_pair :
   ...
   fun inh_left inh_right r ->
    conde [ fresh (l r) (r === Std.pair l r) (inh_left l) (inh_right r) ]
  ...
  *)
  ...
  let () =
    run one
      (fun q -> fresh h (inhabit_list (Std.nat 2) inhabit_free q))
      (fun r -> r#reify (Std.List.reify OCanren.reify))
    |> OCanren.Stream.mapi (fun i x ->
           Format.printf "%d:\t%a\n%!" i
             GT.(fmt Std.List.logic @@ fmt OCanren.logic @@ fmt bool)
             x)
  ...
  let () =
    run one
      (fun q -> fresh h (inhabit_list (Std.nat 1) inhabit_free q))
      (fun r -> r#reify (Std.List.reify OCanren.reify))
    |> OCanren.Stream.mapi (fun i x ->
           Format.printf "%d:\t%a\n%!" i
             GT.(fmt Std.List.logic @@ fmt OCanren.logic @@ fmt bool)
  ...
  let () =
    ...
    run one
      (fun q -> fresh h (inhabit_pair_lists (Std.nat 4) inhabit_int q))
      (fun r ->
        r#reify
          (Std.Pair.reify
  ...
  let run_hacky ?(n = 10) patterns2 =
    ...
    let injected_exprs =
      let demo_exprs =
        ...
        let prjl e =
          Std.List.prjc prj1 (fun _ _ -> failwith "should not happen2") e
        in
        let prjp e =
        ...
        let rec hack_list = function
          ...
          | Std.List.Cons (_, tl) ->
              EConstr
                ( "cons",
                  Std.List.of_list id
                    [ EConstr ("int", Std.List.Nil); hack_list tl ] )
        in
    ...
    runR IR.reify IR.show IR.show_logic n q qh
      ( "ideal_IR",
        fun ideal_IR ->
          let init =
            fresh (th el)
              (IR.iftag !!"nil" Matchable.(field (s (z ())) (scru ())) th el
              === ideal_IR)
          in
          List.fold_left
            (fun acc (scru : Expr.injected) ->
